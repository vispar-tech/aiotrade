"""Simple performance benchmark for BybitClientsCache."""

import asyncio
import time
from typing import Dict

from aiotrade import BybitClient, BybitClientsCache
...
async def benchmark_direct_creation_gather(client_count: int) -> float:
    ...
    async def create_client(i: int) -> BybitClient:
...
async def benchmark_cache_get_or_create_gather(client_count: int) -> float:
    ...
    async def get_or_create_client(i: int) -> BybitClient:
...
async def benchmark_cache_get_gather(client_count: int) -> float:
    ...
    async def get_client(i: int) -> BybitClient:
        api_key, api_secret = make_credentials(i, "cache_warm_gather")
        client = BybitClientsCache.get(
            api_key=api_key, api_secret=api_secret, testnet=True
        )
...
async def main() -> None:
    ...
    print("BybitClientsCache Performance Benchmark")
    print("=" * 40)
